...
export default function MembersView({ isManager, messId }: { isManager: boolean, messId?: string }) {
  ...
  return (
    <div className="flex flex-col h-full relative z-10 w-full animate-fade-in">
      ...
      <AnimatePresence>
        {selectedMember && (() => {
          const joinedAt = (selectedMember as any).joinedAt?.toDate?.() || (selectedMember as any).createdAt?.toDate?.() || (selectedMember.joinDate ? new Date(selectedMember.joinDate) : null);
          const formattedTime = joinedAt 
            ? joinedAt.toLocaleString('bn-BD', { 
                year: 'numeric', 
                month: 'long', 
