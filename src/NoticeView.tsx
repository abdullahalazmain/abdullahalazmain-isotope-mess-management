                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-4">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${notice.isUrgent ? 'bg-rose-500 text-white' : 'bg-indigo-100 text-indigo-600'}`}>
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                           ...
                           {notice.isUrgent && (
                             <span className="px-2 py-0.5 bg-rose-100 text-rose-600 text-[8px] font-black uppercase rounded-full border border-rose-200">Urgent</span>
                           )}
                        </div>
                        <p className="text-xs font-black text-slate-700">ম্যানেজার থেকে</p>
                      </div>
                    </div>
                    ...
                    <p className="text-base font-semibold text-slate-600 leading-relaxed max-w-4xl">
                      {notice.description}
                    </p>
                      ...
                      <div className="mt-8 flex items-center gap-4">
                        {!isRead ? (
                          <button 
                            ...
                            className="px-6 py-2.5 bg-indigo-600 text-white text-xs font-black rounded-xl shadow-lg shadow-indigo-200 hover:bg-indigo-700 hover:scale-105 transition-all flex items-center gap-2"
                          >
                            <Check className="w-4 h-4" /> মার্ক এজ রিড
                          </button>
                        ) : (
                          <button 
                            onClick={() => handleDelete(notice.id)}
                            className="px-6 py-2.5 bg-slate-100 text-slate-600 text-xs font-black rounded-xl hover:bg-rose-50 hover:text-rose-500 transition-all flex items-center gap-2"
                          >
                    ...
                    {isManager ? (
                      <>
                        <div className="flex flex-col items-center">
                          <div className="text-2xl font-black text-indigo-600 leading-none">{notice.readBy?.length || 0}</div>
                          <p className="text-[9px] font-black text-slate-400 uppercase">পঠিত</p>
                        </div>
                        <button 
                          onClick={() => handleDelete(notice.id)}
                          className="w-12 h-12 rounded-2xl bg-rose-50 text-rose-500 flex items-center justify-center hover:bg-rose-500 hover:text-white transition-all shadow-sm hover:rotate-12"
                        >
                      ...
                      isRead && (
                        <div className="hidden md:flex flex-col items-center text-emerald-500 bg-emerald-50 p-3 rounded-2xl border border-emerald-100">
                          <CheckCircle2 className="w-6 h-6" />
                          <p className="text-[8px] font-black uppercase mt-1">পড়া হয়েছে</p>
                        </div>
                      )
                    )}
                  </div>
